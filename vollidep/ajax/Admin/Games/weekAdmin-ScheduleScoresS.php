			...
			$sql2 = "select * from (
            						...
            						select 1 as VisitanteS, 
										...
										case 
											when jugado = 0 then 
												concat('<select name=''jugado',Juego_ID,''' id=''jugado',Juego_ID,'''><option value=''0'' selected>" . $lang['660'] . "</option><option value=''1''>" . $lang['661'] . "</option><option value=''2''>" . $lang['662'] . "</option></select>')
											when jugado = 1 then 
												concat('<select name=''jugado',Juego_ID,''' id=''jugado',Juego_ID,'''><option value=''0''>" . $lang['660'] . "</option><option value=''1'' selected>" . $lang['661'] . "</option><option value=''2''>" . $lang['662'] . "</option></select>')
											when jugado = 2 then 
												concat('<select name=''jugado',Juego_ID,''' id=''jugado',Juego_ID,'''><option value=''0''>" . $lang['660'] . "</option><option value=''1''>" . $lang['661'] . "</option><option value=''2'' selected>" . $lang['662'] . "</option></select>')
										end as jugado,
										j.Local_ID, 
										case when j.Visitante_Id is null then '' else concat(l.equipo_desc,'') end  as 'Local', 
            							case when j.Visitante_ID is null then null when jugado = 0 then '' else CONCAT(Gol_Local,'') end as 'Goles Local', 
				...
				if ($result2->num_rows > 0) {
					...
					while($row2 = $result2->fetch_assoc()) {
						...
						if (strpos($row2["Comentarios"],$lang['654']) !== false){
							...
							$htmlWeek .= '<td scope="row"><div class="d-flex px-2 py-1"><div style="width: 120px;text-align: right;padding-right: 3px;padding-top: 6px;">' . $row2["Local"] . '</div>
								<div>' . $row2["Logol"] . '</div>
							...
							if (strpos($row2["Comentarios"],$lang['654']) !== false){
								...
								$htmlWeek .= '<div class="form-check" style="padding-left: 0px;">
												<input class="form-check-input form-check-input-sm" type="checkbox" value="" name="penall' . $row2["juego"] . '" id="penall' . $row2["juego"] . '" ' . $row2["marcadorpl"] . '>
							...
							if (strpos($row2["Comentarios"],$lang['654']) !== false){
								...
								$htmlWeek .= '<div class="input-group input-group-sm input-group-outline my-0">
												<input type="text" style="width: 22px;text-align: center;padding-right: 0px !important;padding-left: 0px !important;" class="form-control form-control-sm" value="' . $row2["Goles Local"] . '" name="golesl' . $row2["juego"] . '" id="golesl' . $row2["juego"] . '">
											</div>';
							}
							$htmlWeek .= '</div>
							...
							if (strpos($row2["Comentarios"],$lang['654']) !== false){
								...
								$htmlWeek .= '<div class="input-group input-group-sm input-group-outline my-0">
												<input type="text" style="width: 22px;text-align: center;padding-right: 0px !important;padding-left: 0px !important;" class="form-control form-control-sm" value="' . $row2["Goles Visitante"] . '" name="golesv' . $row2["juego"] . '" id="golesv' . $row2["juego"] . '">
											</div>';
							}
							$htmlWeek .= '</div>
								<div ' .  $Config->EmpatesPenales . ' style="width: 20px;text-align: center;padding-top: 6px;">';
							if (strpos($row2["Comentarios"],$lang['654']) !== false){
								...
								$htmlWeek .= '<div class="form-check" style="padding-left: 0px;">
												<input class="form-check-input form-check-input-sm" type="checkbox" value="" name="penalv' . $row2["juego"] . '" id="penalv' . $row2["juego"] . '" ' . $row2["marcadorpv"] . '>
												<label class="custom-control-label" for="penall"></label>
											</div>';
							}
							$htmlWeek .= '</div>
								<div>' . $row2["Logov"] . '</div>
								<div style="width: 120px;text-left: right;padding-left: 3px;padding-top: 6px;">' . $row2["Visitante"] .'</div></div></td>';
						}
						if (strpos($row2["Comentarios"],$lang['654']) !== false){
				...
				$sql33 = "SELECT distinct a.Equipo_ID, 
								Equipo_DESC 
			...
			$sql2 = "select * from (
            						select 0 as VisitanteS, 
            							...
            							case when j.Visitante_ID is null then null else Arbitro end as Arbitro, 
            							case 
            								when j.Visitante_Id is null then CONCAT('<div class=''circularsmall'' style=''height: 30px;width: 30px;border-radius:7px;margin-right: 5px;     text-align: left;''><img src=''imagenes/', concat(l.Torneo_ID,'-', l.Equipo_ID), '.png'' width=''30'' height=''30'' alt='''' style=''vertical-align:middle !important;''/></div><div style=''padding-top: 6px;''><span>',l.equipo_desc,' " . $lang['654'] . "</span></div>') 
            								else Comentarios 
            							end as Comentarios, 
            							Estatus, 
            							case when j.Visitante_ID is null then null else ifnull(jc.Campo_DESC, lc.Campo_DESC) end as Campo, 
            							...
            							case 
            								when j.Visitante_Id is null then ''
            									...
            									CONCAT('<div class='''' style=''height: 30px;width: 30px;''><img src=''imagenes/',concat(l.Torneo_ID,'-', l.Equipo_ID),'.png'' width=''30'' height=''30'' alt=''''/></div>') 
            							end as Logol, 
            							case 
            								when j.Visitante_Id is null then ''
            								else
            									CONCAT('<div class='''' style=''height: 30px;width: 30px; text-align: left;''><img src=''imagenes/',concat(v.Torneo_ID,'-', v.Equipo_ID),'.png'' width=''30'' height=''30'' alt=''''/></div>') 
            							end  as Logov,
            							case 
            								when jugado = 0 then '' 
            								else '-'
            						...
            						select 1 as VisitanteS, 
										...
										case when j.Visitante_ID is null then null else Arbitro end as Arbitro, 
            							case 
            								when j.Visitante_Id is null then 	CONCAT('<img src=\"imagenes/', concat(l.Torneo_ID,'-', l.Equipo_ID), '.png\" class=\"avatar avatar-sm me-0\" style=\"border-radius: 0rem !important;\"/>',l.equipo_desc,' " . $lang['654'] . "')
            								else Comentarios 
            							end as Comentarios, 
            							Estatus, 
										case when j.Visitante_ID is null then null else ifnull(jc.Campo_DESC, lc.Campo_DESC) end as Campo, 
				...
				if ($result2->num_rows > 0) {
					...
					while($row2 = $result2->fetch_assoc()) {
						...
						if (strpos($row2["Comentarios"],$lang['654']) !== false){
							...
							$htmlWeek .= '	<div class="align-self-center" style="width: 30%; text-align: right;padding-right: 3px; font-size:3vw;">' . $row2["Local"] . '</div>
							...
							$htmlWeek .= '	</div>';
							$htmlWeek .= '	<div style="width: 20px;text-align: right;" class="align-self-center">';
							$htmlWeek .= '		<div class="input-group input-group-sm input-group-outline my-0">
													<input type="text" style="width: 22px;text-align: center;padding-right: 0px !important;padding-left: 0px !important;" class="form-control form-control-sm" value="' . $row2["Goles Local"] . '" name="golesl' . $row2["juego"] . '" id="golesl' . $row2["juego"] . '">
												</div>';
							$htmlWeek .= '	</div>
							...
							$htmlWeek .= '	</div>';
							$htmlWeek .= '	<div ' .  $Config->EmpatesPenales . ' style="width: 20px;text-align: center;" class="align-self-center">';
							$htmlWeek .= '		<div class="form-check" style="padding-left: 0px;">
													<input class="form-check-input form-check-input-sm" type="checkbox" value="" name="penalv' . $row2["juego"] . '" id="penalv' . $row2["juego"] . '" ' . $row2["marcadorpv"] . '>
													<label class="custom-control-label" for="penall"></label>
												</div>';
							$htmlWeek .= '	</div>
											<div class="align-self-center">' . $row2["Logov"] . '</div>
						...
						if (strpos($row2["Comentarios"],$lang['654']) !== false){
							...
							$htmlWeek .= '<div style="width: 23%;text-align: center;padding-top: 6px;" ' .  $Config->time . '>
						...
						if (strpos($row2["Comentarios"],$lang['654']) !== false){
							...
							//if($row2["jugadoStat"] == 1){
								$htmlWeek .= '<div style="width: 8%;text-align: right;padding-top: 6px;">
									<p style="margin-bottom: 0rem !important;"><img class="expandirButton" id="expandir' . $row2["juego"] . 'SA" src="./imagenes/expandir.png" height="25" width="25" onClick="abrirFichaEditS(' . $row2["juego"] . ', ' . $row2["Jornada"] . ', ' . $row2["juego"] . ', \'' . $row2["Local"] . ' vs ' . $row2["Visitante"] . '\', \'' . $row2["Goles Local"] . '\', \'' . $row2["Goles Visitante"] . '\'); "></p>
									</div>';
							//}else{
							//	$htmlWeek .= '<div style="width: 8%;text-align: right;padding-top: 6px;">
							//		<p style="margin-bottom: 0rem !important;"><span class="text-secondary text-xs font-weight-bold"></span></p>
