...
set_time_limit(300);
	...
	$jugadores = $_SESSION[$Config->getAlias() . 'printList'];
	...
	$sql = "SELECT Jugador_ID,
					...
					FROM $schema.Jugadores a
						join $schema.Equipos b on a.Equipo_ID = b.Equipo_ID and b.Torneo_ID = (select Torneo_ID from $schema.Torneos where Actual = 'S')
						join $schema.Categorias c on b.Fuerza = c.Categoria_ID
						join $schema.Colores d on c.Color = d.Color_HEX
					where Jugador_ID in ($jugadores) and a.Foto is not null and YEAR(CURDATE())-YEAR(Fecha_Nacimiento) between $edad1 and $edad2
					order by a.Equipo_ID asc, convert(Numero,unsigned) asc";
