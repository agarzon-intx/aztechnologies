...
unset($__i, $__prev, $__base, $__inc, $__app_here);
	...
	$sessionstat = $fgmembersite->CheckLogin('RefereeManagementEditSave.php');
    ...
    $refereeid = SanitizeText($_POST['refereeid']); 
	...
	if(strlen($idf) > 0){
		if(strlen($idf) > 0){
			...
			if (file_exists("refereePictureNew-" . session_id() . ".png")) { unlink ("refereePictureNew-" . session_id() . ".png"); }
	...
	$retunData = array('status' => '0', 'message' => 'No insert.', 'dataRefereeMessage' => 'Error');

	$sql = "CALL $schema.RefereeUpdate('" . $_SESSION[$Config->getAlias() . 'username'] . "', $refereeid, '$name', '$lastname', '$lastname2', '$nickname', '$birthdate', '$status', '$id', '$valid', '$comments', '$record' , '$courses' , '$phone', '$email','$foto','$idfull','$firma',$sex, NOW(), @out);";
	//echo $sql;
	...
	if ($result->num_rows > 0) {
		...
		while($row2 = $result->fetch_assoc()) {
			$retunData = array('status' => '1', 'message' => 'Success.', 'dataRefereeMessage' => $lang['938'], 'sql' => $sql);
		}
	}
	$Connection->Close();
