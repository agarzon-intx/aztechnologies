...
unset($__i, $__prev, $__base, $__inc, $__app_here);
	...
	$sessionstat = $fgmembersite->CheckLogin('RefereeManagementCreateSave.php');
    ...
    $record = SanitizeText($_POST['record']);
    $courses = SanitizeText($_POST['courses']);
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
				$arbitroexistente = $lang['10757'] ;
		}		
	}
	
	if($found == 1){
		$retunData = array('status' => '0', 'message' => 'exist.', 'dataRefereeMessage' => $arbitroexistente);
		...
		if(strlen($picture) > 0){
			...
			if ($handle->uploaded) {
				...
				$handle->file_new_name_body      = "refereePictureNew-" . session_id();
				$handle->file_new_name_ext       = "png";
				$handle->Process($target_dir);
					
				$handle-> Clean();
			}
			$foto=addslashes(file_get_contents("refereePictureNew-" . session_id() . ".png"));
		...
		if(strlen($idf) > 0){
			if(strlen($idf) > 0){
				...
				if (file_exists("refereerPictureNew-" . session_id() . ".png")) { unlink ("refereePictureNew-" . session_id() . ".png"); }
    	...
    	$retunData = array('status' => '0', 'message' => 'No insert.', 'dataRefereeMessage' => 'Error', 'sql0' => $sql0);

		$sql0 = "CALL $schema.RefereeCreate('" . $_SESSION[$Config->getAlias() . 'username'] . "', '$clave', '$name', '$lastname', '$lastname2', '$nickname', '$birthdate', '$status', '$id', '$valid', '$comments', '$record', '$courses', '$phone', '$email','$foto','$idfull','$firma',$sex, NOW(), NOW(), @out);";
    	...
    	if ($result->num_rows > 0) {
    		$found = 1;
    		while($row = $result->fetch_assoc()) {
    				$retunData = array('status' => '1', 'message' => 'Success.', 'dataRefereeMessage' => $lang['10716'], 'sql0' => mb_convert_encoding($sql0, 'UTF-8', 'UTF-8'));
    		}		
    	}
		$Connection->Close();
