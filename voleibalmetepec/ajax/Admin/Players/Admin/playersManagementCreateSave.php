...
unset($__i, $__prev, $__base, $__inc, $__app_here);
	...
	$sql="	SELECT b.Equipo_FULLDESC, c.Categoria_Desc
			...
			where Curp like '$id' and b.Torneo_ID in (select max(a.Torneo_ID) 
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
				$jugadorexistente = $lang['957'] . $row["Equipo_FULLDESC"] . $lang['958'] . $row["Categoria_Desc"];
		}		
	}
	
	if($found == 1){
		$retunData = array('status' => '0', 'message' => 'exist.', 'dataPlayerMessage' => $jugadorexistente);
    	...
    	//$sql1 = "CALL $schema.PlayerCreateMulti('" . $_SESSION[$Config->getAlias() . 'username'] . "', '$clave', '$playernumber', $team, @out);";
	...
	//	$Connection->Close();
		...
		$sql0 = "CALL $schema.PlayerCreate('" . $_SESSION[$Config->getAlias() . 'username'] . "', '$clave', '$name', '$lastname', '$lastname2', '$nickname', '$birthdate', '$status', '$id', '$playernumber', $team, '$valid', '$comments', '$phone', '$email','$foto','$idfull','$firma',$sex, $type, NOW(), NOW(), @out);";
		
		$Connection = $Config->connectAdmin();
		$result = $Config->queryAdmin($sql0);
