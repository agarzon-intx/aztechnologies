	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			try{
				...
				try{
					$pdf->SetAlpha(1);
					$pdf->Image($server . '/imagenes/' . $Config->logo . '.png',$x+2+((15.68 - (15.68 * ($Config->logowidth / 110)))/2)-18.7,$y-30+5+((15.68 - (15.68 * ($Config->logoheight / 110)))/2)+28.45,(15.68 * ($Config->logowidth / 110)), (15.68 * ($Config->logoheight / 110)), 'PNG');
				}catch(Exception $e){
					echo $e;
				}
				...
				try{
					$pdf->SetAlpha(1);
					$pdf->Image($server . '/Form/fetch_image.php?Jugador_ID=' . $row["Jugador_ID"] . '&Imagen=Foto',$x+2-18.7,$y-30+30+28.45,15.68, 20.38, 'PNG');
				}catch(Exception $e){
					try{
    					$pdf->SetAlpha(1);
    					$pdf->Image($server . '/Form/fetch_image.php?Jugador_ID=' . $row["Jugador_ID"] . '&Imagen=Foto',$x+2-18.7,$y-30+30+28.45,15.68, 20.38, 'JPG');
    				}catch(Exception $e){
    					try{
        					$pdf->SetAlpha(1);
        					$pdf->Image($server . '/Form/fetch_image.php?Jugador_ID=' . $row["Jugador_ID"] . '&Imagen=Foto',$x+2-18.7,$y-30+30+28.45,15.68, 20.38, 'JPEG');
        				}catch(Exception $e){
        					try{
            					$pdf->SetAlpha(1);
            					$pdf->Image($server . '/Form/fetch_image.php?Jugador_ID=' . $row["Jugador_ID"] . '&Imagen=Foto',$x+2-18.7,$y-30+30+28.45,15.68, 20.38, 'GIF');
            				}catch(Exception $e){
            					echo $e->getMessage();
            				}
        				}
				...
				try{
					$pdf->SetAlpha(1);
					$pdf->Image($server . '/imagenes/' . $row["Logo"] . '.png',$x+3-18.7,$y-30+54.5+28.45,15.68, 15.68, 'PNG');
				}catch(Exception $e){
					echo $e;
				}
				...
				try{
					$pdf->SetAlpha(1);
					$pdf->Image($server . '/imagenes/Aztechnologies-S.png',$x+20.5-18.7,$y-30+78.08+28.45,33, 9.42, 'PNG');
				}catch(Exception $e){
					echo $e;
				}	
				try{
					$pdf->SetAlpha(.3);
					$pdf->Image($server . '/imagenes/Aztechnologies-A.png',$x+20.5-18.7,$y-30+27.5+28.45,33, 41.5, 'PNG');
				}catch(Exception $e){
					echo $e;
				}				
				if($col == 4){
