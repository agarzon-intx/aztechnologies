<?php
	...
	$sql = "SELECT 	Jugador_ID, 
			...
			FROM $schema.Jugadores a 
				...
				join $schema.Categorias c on b.Fuerza = c.Categoria_ID and c.Torneo_Id = $torneo AND b.Torneo_ID= c.Torneo_ID
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			try{
				...
				try{
					$pdf->SetAlpha(.4);
					az_pdf_image_file($pdf, $siteRoot, '/imagenes/VersionDePrueba.png', $x,$y+5,65, 100);
				}catch(Exception $e){
					echo $e;
				}
				//$pdf->Table();
