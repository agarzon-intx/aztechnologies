<?php
	...
	$sql = "select dc.Categoria_DESC, SUBSTRING(jor.Jornada_Desc, 1, 4) Jornada_DescCorta, a.Juego_ID, a.Local_ID, d.Equipo_FULLDESC as Local, a.Visitante_ID, f.Equipo_FULLDESC as Visitante, a.Fecha, day(a.Fecha) Dia, 
			...
			where a.Torneo_ID = $torneo and (d.Equipo_FULLDESC like '%" . $equipo . "%' or f.Equipo_FULLDESC like '%" . $equipo . "%')
	...
	if ($result1->num_rows > 0) {
		...
		while($row1 = $result1->fetch_assoc()) {
        	...
        	$height = 1;
        	if(strlen($row1["Comentarios"])/65 > 1){
        	    $height = intdiv(strlen($row1["Comentarios"]), 65);
        	    if(strlen($row1["Comentarios"])%65 > 0){
        	       $height++;
        	    }
        	}
        	$pdf->SetTextColor(0, 0, 0);
        	...
        	$pdf->SetXY($x+168,$y+3);
        	if(strlen($row1["Comentarios"])/65 > 1){
        	    $pdf->MultiCell(70, (3), substr_replace(az_utf8_decode($row1["Comentarios"]), "\n", strpos(az_utf8_decode($row1["Comentarios"]), " ", 65), 0), 1,'L', false);
        	}else{
