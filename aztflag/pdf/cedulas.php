<?php
	...
	$sql = "select dc.Categoria_DESC, jor.Jornada_DescCorta, a.Juego_ID, a.Local_ID, d.Equipo_FULLDESC as Local, a.Visitante_ID, f.Equipo_FULLDESC as Visitante, b.Fecha, day(b.Fecha) Dia, 
			...
			where a.Torneo_ID = $torneo and b.Jornada_ID = $jornada $categoriaFilter and ((weekday(a.Fecha) <> 2) or (weekday(a.Fecha) = (SELECT MarcadorDiaDefault-1 FROM $schema.Configuration) and a.Horario <> (SELECT MarcadorHoraDefault FROM $schema.Configuration)))
	...
	if ($result1 && $result1->num_rows > 0) {
		...
		while($row1 = $result1->fetch_assoc()) {
			...
			$pdf->SetXY($x+6,$y+46);
			$pdf->Cell(100 , 5, mb_strtoupper('' . az_utf8_decode($row1["Local"]) . ''), 1, 1 , 'C' , false);
			...
			$pdf->SetFont('Times' , 'B' , 12);
			$pdf->SetXY($x+10,$y+46);
			$pdf->Cell(100 , 5, mb_strtoupper ('' . az_utf8_decode($row1["Visitante"]) . ''), 1, 1 , 'C' , false);
			$pdf->SetFont('Times' , '' , 10);
			$y = $y + 4;
			$pdf->SetXY($x+10,$y+47);
			$pdf->Cell(100 , 128, '', 1, 1 , 'C' , false);
