<?php
				...
				$htmlWeek .= '<th scope="col" class="text-center text-uppercase text-secondary text-s font-weight-bolder opacity-7" style="padding: 0.75rem 0.5rem;"><img src="./imagenes/amarilla.png" width="20" height="20" alt=""></th>';
				$htmlWeek .= '</thead>';
				$htmlWeek .= '<tbody>';
				
				
				$sql2 = "select distinct a.* from (select a.Jornada_ID, Jornada_DescCorta, concat(b.Nombre,' ', b.Apellido_P, ' ', b.Apellido_M) Jugador, c.Equipo_DESC, concat(c.Torneo_ID,'-', c.Equipo_ID) Logo, Cantidad Amarillas 
						from $schema.Amonestados a 
					 ...
					 if ($result2->num_rows > 0) {
						...
						while($row2 = $result2->fetch_assoc()) {
							...
							$htmlWeek .= '<td scope="row" class="align-middle text-left"><div class="d-flex px-0 py-1"><div style="width: 40px;text-align: right;padding-right: 3px;"><img src="imagenes/' . $row2["Logo"]. '.png?tmp=' . $fecha->getTimestamp() . '" class="avatar avatar-sm me-0" style="border-radius: 0rem !important;"></div><div style="width: 100px;text-align: left;padding-right: 3px;padding-top: 6px;"><span class="text-secondary text-xs font-weight-normal">' . $row2["Equipo_DESC"]. '</span></div></div></td>
											...
											<td scope="row" class="align-middle text-center"><span class="text-secondary text-xs font-weight-normal">' . $row2["Amarillas"]. '</span></td>';
							$htmlWeek .= '</tr>';
							$count++;
						}
					}
				...
				$htmlWeek .= '<th scope="col" class="text-center text-uppercase text-secondary text-s font-weight-bolder opacity-7" style="padding: 0.15rem 0.1rem;"><img src="./imagenes/amarilla.png" width="20" height="20" alt=""></th>';
				$htmlWeek .= '</thead>';
				$htmlWeek .= '<tbody>';
					 ...
					 if ($result2->num_rows > 0) {
						...
						while($row2 = $result2->fetch_assoc()) {
							...
							$htmlWeek .= '<td scope="row" class="align-middle text-left" style="padding: 0.15rem 0.1rem;"><div class="d-flex px-0 py-1"><div style="width: 40px;text-align: right;padding-right: 3px;"><img src="imagenes/' . $row2["Logo"]. '.png?tmp=' . $fecha->getTimestamp() . '" class="avatar avatar-sm me-0" style="border-radius: 0rem !important;"></div></div></td>
											...
											<td scope="row" class="align-middle text-center" style="padding: 0.15rem 0.1rem;"><span class="text-secondary text-xs font-weight-normal">' . $row2["Amarillas"]. '</span></td>';
							$htmlWeek .= '</tr>';
							$count++;
						}
					}
