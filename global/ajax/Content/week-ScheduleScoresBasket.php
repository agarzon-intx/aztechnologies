			...
			$sql2 = "select * from (
								select 0 as VisitanteS, 
										...
										concat('https://www.google.com/maps/@', ifnull(jc.lat,lc.lat), ',', ifnull(jc.lon,lc.lon), ',', ifnull(jc.zoom,lc.zoom) ,'z/data=!3m1!1e3') as Google, 
										case 
									...
									select 1 as VisitanteS, j.Torneo_ID as Torneo, jo.Jornada_ID as Jornada, j.Juego_ID as juego, jugado, case when j.Visitante_Id is null then '' else concat(l.equipo_desc,'') end  as 'Local', 
										...
										Estatus, case when j.Visitante_ID is null then null else ifnull(jc.Campo_DESC, lc.Campo_DESC) end as Campo, 
										concat('https://www.google.com/maps/@', ifnull(jc.lat,lc.lat), ',', ifnull(jc.lon,lc.lon), ',', ifnull(jc.zoom,lc.zoom) ,'z/data=!3m1!1e3') as Google, 
										case 
											when j.Visitante_Id is null then ''
											else
				...
				if ($result2->num_rows > 0) {
					...
					while($row2 = $result2->fetch_assoc()) {
						...
						$htmlWeek .= '
							<td scope="row"><div class="d-flex px-2 py-1"><div style="width: 120px;text-align: right;padding-right: 3px;padding-top: 6px;">' . $row2["Local"] . '</div>
							<div>' . $row2["Logol"] . '</div>';
						if($row2["jugado"] == 2){
							$htmlWeek .= '<div style="width: 80px;text-align: center;padding-top: 6px;"><span style="font-size: small;">' . $lang['663'] . '</span></div>';
						}
						if($row2["jugado"] == 1){
						    if($row2["PL"] > $row2["PV"]){
						        $colorSL = 'green';
						...
						$htmlWeek .= '<div>' . $row2["Logov"] . '</div>
							...
							$htmlWeek .= '<td  scope="row" class="align-middle text-center">';
								if (strpos($row2["Comentarios"],$lang['654']) !== false){
									...
									if($row2["jugado"] == 1){
										$htmlWeek .= '<img class="expandirButton" id="expandir' . $row2["juego"] . '" src="./imagenes/expandir.png" height="25" width="25" onClick="abrirFichaBasket(' . $row2["juego"] . ', ' . $row2["Jornada"] . ', ' . $row2["juego"] . ', \'' . $row2["Local"] . ' vs ' . $row2["Visitante"] . '\', \'' . $row2["Goles Local"] . '\', \'' . $row2["Goles Visitante"] . '\'); ">';
									}
								}
							$htmlWeek .= '</td>';
						$htmlWeek .= '</tr>';
				...
				if ($result2->num_rows > 0) {
					...
					while($row2 = $result2->fetch_assoc()) {
						...
						$htmlWeek .= '
							<td scope="row"><div class="justify-content-center d-flex px-0 py-1"><div class="align-self-center" style="width: 30%; text-align: right;padding-right: 3px; font-size:3vw;">' . $row2["Local"] . '</div>
						...
						if($row2["jugado"] == 1){
							...
							$htmlWeek .= '
							<div style="width: 30px;text-align: right;color: ' . $colorSL . ';" class="align-self-center">' . $row2["PL"] . '</div>
							<div style="width: 10px;text-align: center;" class="align-self-center">' . $row2["marcador"] . '</div>
							<div style="width: 30px;text-align: left;color: ' . $colorSV . ';" class="align-self-center">' . $row2["PV"] . '</div>';
						}
						if($row2["jugado"] == 0){
							...
							$htmlWeek .= '
							<div style="width: 30px;text-align: right;color: ' . $colorSL . ';" class="align-self-center"></div>
							<div style="width: 10px;text-align: center;" class="align-self-center"></div>
							<div style="width: 30px;text-align: left;color: ' . $colorSV . ';" class="align-self-center"></div>';
						}
						
						
						$htmlWeek .= '<div class="align-self-center">' . $row2["Logov"] . '</div>
							<div style="width: 30%; text-align: left; padding-left: 3px; font-size:3vw;" class="align-self-center">' . $row2["Visitante"] .'</div></div><div class="d-flex px-0 py-1">';
						if (strpos($row2["Comentarios"],$lang['654']) !== false){
							$htmlWeek .= "";
						}else{
						...
						$htmlWeek .= '';
							if (strpos($row2["Comentarios"],$lang['654']) !== false){
								...
								if($row2["jugado"] == 1){
									...
									<img class="expandirButton" id="expandir' . $row2["juego"] . 'S" src="./imagenes/expandir.png" height="25" width="25" onClick="abrirFichaSBasket(' . $row2["juego"] . ', ' . $row2["Jornada"] . ', ' . $row2["juego"] . ', \'' . $row2["Local"] . ' vs ' . $row2["Visitante"] . '\', \'' . $row2["Goles Local"] . '\', \'' . $row2["Goles Visitante"] . '\'); "></div>';
								}
							}
						$htmlWeek .= '</div></td>';
						$htmlWeek .= '</tr>';
