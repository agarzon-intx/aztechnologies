            <?php
				...
				$htmlTeam .= '<th scope="col" class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7" style="padding: 0.75rem 0.5rem;"><img src="imagenes/gamePlayedVoleibol.png" width="20" height="20" alt=""/></th>';
				...
				$sql3 = "select @rank:=@rank+1 AS rank, a.* 
						 from (SELECT distinct a.Jugador_ID,
										...
										DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),a.Fecha_Nacimiento)), '%Y') + 0 AS Edad, 
				 ...
				 if ($result2->num_rows > 0) {
					...
					if($row2["Sexo"] == 0){ 
						...
						if($Config->perfiljugador == 1){
            	            $htmlTeam .=  '<img src="imagenes/lupa.png" width="15" height="15" alt="" onclick="previewPlayerShowVoleibol(' . $row2["Jugador_ID"] . ')">';
            	            }
						$htmlTeam .=  ' </span></td>
						        </tr>';
						$count++;
				...
				$sql2 = "select @rank:=@rank+1 AS rank, a.* 
						 from (SELECT distinct a.Jugador_ID,
									...
									FROM $schema.Jugadores a
										LEFT OUTER JOIN (select Jugador_ID, sum(Jugado) Juegos from $schema.JugadorJugado where Torneo_ID = $Season group by Jugador_ID) e on a.Jugador_ID = e.Jugador_ID 
										LEFT OUTER JOIN $schema.Range f on f.Range_Active = 1 and f.Range_Id <> 1 and year(now())-year(a.Fecha_Nacimiento) between f.Range_Start and f.Range_End
										LEFT OUTER JOIN $schema.Colores g on f.Range_Color_ID = g.Color_ID
										LEFT OUTER JOIN $schema.Categorias h on year(now())-year(a.Fecha_Nacimiento) between h.Edad_Inicial and h.Edad_Final
										JOIN $schema.Equipos i ON a.Equipo_ID = i.Equipo_ID and i.Torneo_ID = $Season
										JOIN $schema.Categorias j ON i.Fuerza = j.Categoria_ID 
