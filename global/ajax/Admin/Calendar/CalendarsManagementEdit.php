...
unset($__i, $__prev, $__base, $__inc, $__app_here);
	...
	$sessionstat = $fgmembersite->CheckLogin('CalendarsManagementEdit.php');
	
	include("class.upload.php");
	include('lang.'.$_COOKIE[$Config->getAlias() . 'language'].'.php');
	
	$calendar_id = SanitizeInteger($_POST['calendar']);

	$retunData = array('status' => '0', 'message' => 'Something went wrong,please try again.');

	$htmlCalendar = "";

	$sql="	SELECT Calendario_ID,
                    Calendario_DESC 
            FROM $schema.Calendario
			where Calendario_ID = $calendar_id;";
	$result = $Config->query($sql);
	if ($result->num_rows > 0) {
		while($row2 = $result->fetch_assoc()) {
			$descripcion = $row2["Calendario_DESC"];
		}		
	}
	
	$htmlCalendar = "";
	$htmlCalendar .= '<div class="container-fluid py-2">
					<div class="row">
						<div class="col-xl-12">
							<h3>' . $lang['66-1'] . '</h3>
						</div>
								...
								<div class="row">
									<div class="col-xl-12" hidden>
										<div class="input-group input-group-outline my-3" style="margin-top: 5px !important;margin-bottom: 0px !important;">
											<label class="form-label">' . $lang['51-1'] . '</label>
											<input type="text" class="form-control" name="categoryid2" id="categoryid2" value=""/>
										</div>
									</div>
									<div class="col-xl-12">
										<div class="input-group input-group-outline my-3" style="margin-top: 5px !important;margin-bottom: 0px !important;">
											<label class="form-label">' . $lang['52-1'] . '</label>
					...
					<div class="row">
						<div class="my-3" >
							<button type="button" class="btn btn-primary" onClick="validateCalendaryEdit(' . $calendar_id . ');" >' . $lang['0000'] . '</button>
							<button type="button" class="btn btn-primary" onClick="calendarManagementHideEdit();" >' . $lang['0001'] . '</button>
						</div>
					</div>
				</div>
				<script>
	...
	$retunData = array('status' => '1', 'message' => 'Success.', 'dataCalendarEdit' => $htmlCalendar);
    $Config->Close();
