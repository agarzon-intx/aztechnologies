<?php
	$fecha = new DateTime();
$sql2Teams = "	SELECT a.*, concat(a.Torneo_ID,'-', a.Equipo_ID) newLogo, c.Campo_DESC, d.categoria_desc
...
if ($result2 && $result2->num_rows > 0) {
	while($row2 = $result2->fetch_assoc()) {
		...
		$htmlTeams .=	'<td scope="row" class="align-middle text-center"><span class="text-secondary text-xs font-weight-normal"><input type="checkbox" class="teamsManagementSelect" data-scope="active" name="teamsManagementSelect' . (int) $row2["Equipo_ID"] . '" id="teamsManagementSelectActive' . (int) $row2["Equipo_ID"] . '" value="' . (int) $row2["Equipo_ID"] . '"/></span></td>
						<td ' . $Config->ShowIDColumn . ' scope="row" class="align-middle text-left"><span class="text-secondary text-xs font-weight-normal">' . $row2["Equipo_ID"]. '</span></td>
	...
	while($row2 = $result2->fetch_assoc()) {
		...
		$htmlTeams .= '<td scope="row" style="padding-top: 0.35rem; padding-bottom: 0.35rem;">
							<div class="d-flex flex-wrap px-0 py-0 w-100" style="min-width:0;">
								<div style="flex: 0 0 10%; max-width:10%; min-width:0; text-align: center; padding-top: 0.5rem;">
									<input type="checkbox" class="teamsManagementSelect" data-scope="active" name="teamsManagementSelectM' . (int) $row2["Equipo_ID"] . '" id="teamsManagementSelectActiveM' . (int) $row2["Equipo_ID"] . '" value="' . (int) $row2["Equipo_ID"] . '"/>
								</div>
								<div ' . $Config->ShowIDColumn . ' style="flex: 0 0 16%; max-width:16%; min-width:0; text-align: center; padding-top: 0px;">
								...
								<div style="flex: 0 0 12%; max-width:12%; min-width:0; text-align: center; padding-top: 0px;">
