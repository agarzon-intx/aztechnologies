...
unset($__i, $__prev, $__base, $__inc, $__app_here);
	...
	$team_id = SanitizeInteger($_POST['team']);
	...
	$sql="	SELECT a.*, c.Campo_DESC 
			...
			WHERE Torneo_ID = (SELECT Torneo_ID FROM $schema.Torneos where Actual = 'S') and 
				a.Equipo_ID = $team_id";
	$result = $Config->query($sql);
	if ($result->num_rows > 0) {
		while($row2 = $result->fetch_assoc()) {
			$equipoidA = $row2["Equipo_ID"]; 
			$descripcionA = $row2["Equipo_DESC"]; 
			if($row2["Activo"] == 1) { 
				$estatusA = "checked"; 
			}else{
				$estatusA = ""; 
			...
			$fuerzaA = $row2["Fuerza"]; 
			$descripcionLargaA = $row2["Equipo_FULLDESC"]; 
			$campoA = $row2["Campo_ID"]; 
			$shortAC = $row2["Short"]; 
								...
								<div class="row">
									<div class="col-xl-12" hidden>
										<div class="input-group input-group-outline my-3" style="margin-top: 5px !important;margin-bottom: 0px !important;">
											...
											<input type="text" class="form-control" name="teamid" id="teamid" value="' . $equipoidA . '"/>
										</div>
									</div>
									<div class="col-xl-12">
										<div class="input-group input-group-outline my-3" style="margin-top: 5px !important;margin-bottom: 0px !important;">
											<label class="form-label">' . $lang['517'] . '</label>
											<input type="text" class="form-control" name="descripcion" id="descripcion" value="' . $descripcionA . '"/>
										</div>
									</div>
									<div class="col-xl-12">
	...
	if ($result3->num_rows > 0) {
		// output data of each row
		while($row3 = $result3->fetch_assoc()) {
			if($row3["Campo_ID"] == $campoA){
				$htmlTeams .= "<option value='" . $row3["Campo_ID"] . "' selected>" . $row3["Campo_DESC"] . "</option>";
			}else{
								...
								<div class="row">
									<div class="col-3 col-sm-3 col-md-3 col-xl-3 col-xxl-3" style="text-align: right;">
										<span style="text-align: center;">
											<img style=" background-color: ' . $playeraAC . ';" id="playera" src="imagenes/uniforme/playera.png" alt="Foto" width="60" height="60"/><span style="text-align: left"></span>
										</span>
									</div>
	...
	if ($result3->num_rows > 0) {
		// output data of each row
		while($row3 = $result3->fetch_assoc()) {
			if($row3["Color_HEX"] == $playeraAC){
				$htmlTeams .= "<option style='background-color:" . $row3["Color_HEX"] . "' value='" . $row3["Color_HEX"] . "' selected>" . $row3["Color_DESC"] . "</option>";
			 }else{
...
$htmlTeams .= '								</select>
								...
								<div class="row">
									<div class="col-3 col-sm-3 col-md-3 col-xl-3 col-xxl-3" style="text-align: right;">
										<span style="text-align: center;">
											<img style=" background-color: ' . $shortAC . ';" id="short" src="imagenes/uniforme/short.png" alt="Foto" width="60" height="40"/><span style="text-align: left"></span>
										</span>
									</div>
...
$htmlTeams .= '								</select>
								...
								<div class="row">
									<div class="col-3 col-sm-3 col-md-3 col-xl-3 col-xxl-3" style="text-align: right;">
										<span style="text-align: center;">
											<img style=" background-color: ' . $calcetasAC . ';" id="calceta" src="imagenes/uniforme/calcetas.png" alt="Foto" width="60" height="40"/><span style="text-align: left"></span>
										</span>
									</div>
									<div class="col-9 col-sm-9 col-md-9 col-xl-9 col-xxl-9" style="text-align: left; padding-top: 7px;">
										<span>
...
$htmlTeams .= '								</select>
									...
									<div class="form-check col-12 col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">
										<span style="text-align: center; ">
											<div style="width: 345px; height: 345px; background-color: #BFBFBF; margin: auto;">
												<img id="logo" src="imagenes/Original/' . $Season . '-' . $equipoidA. '.png?tmp=' . $fecha->getTimestamp() . '" alt="Logo" width="350" height="350"/>
											</div>
										</span>
									</div>
								</div>
					...
					<div class="row">
						<div class="my-3" >
							<button type="button" class="btn btn-primary" onClick="validateTeamEdit(' . $equipoidA . ', ' . $Category . ');" >' . $lang['0000'] . '</button>
							<button type="button" class="btn btn-primary" onClick="teamManagementHideEdit();" >' . $lang['0001'] . '</button>
						</div>
					</div>
				</div>
				<script>
	...
	$retunData = array('status' => '1', 'message' => 'Success.', 'dataTeamEdit' => $htmlTeams);
    $Config->Close();
