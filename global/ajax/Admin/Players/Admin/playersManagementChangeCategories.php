...
unset($__i, $__prev, $__base, $__inc, $__app_here);
					...
					<div class="row">
						<div id="playerAdminContent" class="" style="padding-top: 7px !important;padding-left: 0px !important;padding-right: 0px !important;padding-bottom: 0px !important;">
							<div class="container-fluid" style="padding: 0px;">
								<div class="row" id="playerAdminContentCategory">
									<div class="col-6 col-sm-6 col-md-6 col-lg-6 col-xl-6 col-xxl-6" id="categoriesContent" >
										<div class="container-fluid" style="padding: 0px;">
											<div class="row">
												...
												<div class="col-8 col-sm-8 col-md-5 col-lg-5 col-xl-5 col-xxl-5" id="playerAdminContentCategoryList">';
	
	$sql0 = "select Categoria_ID
	...
	$sql3 = "SELECT distinct a.Fuerza Categoria_ID, b.Categoria_Desc 
			FROM $schema.Equipos a
				join $schema.Categorias b on a.Fuerza = b.Categoria_ID and b.Torneo_ID = $Season
			Where a.Torneo_ID = $Season and a.Fuerza <> $Category and a.Activo = 1
			order by Categoria_Orden asc";
	...
	$htmlCat .= '								</div>
												<div class="col-4 col-sm-4 col-md-2 col-lg-2 col-xl-2 col-xxl-2">
													<div class="dropdowni btn-tooltip" data-bs-toggle="tooltip" data-bs-placement="top">
														<img src="./imagenes/refresh.png" width="20" height="20" onclick="playersManagementAdminCategoryShowReloadList($(\'#playersManagementAdminSelectedCategory\').val());" style="margin-left: 10;  margin-top: 2px;">
													</div>
												</div>
											</div>
										</div>
									...
									<div class="col-6 col-sm-6 col-md-6 col-lg-6 col-xl-6 col-xxl-6">
										...
										<div class="container-fluid" style="padding: 0px;">
											<div class="row">
												...
												<div class="col-8 col-sm-8 col-md-5 col-lg-5 col-xl-5 col-xxl-5" id="playerAdminContentTeamList">';
												
	$team = '';
	$totTeam = 0;
	$sql4 = "SELECT Equipo_ID, Equipo_FULLDESC 
			FROM $schema.Equipos
			where Torneo_ID = $Season 
				and Fuerza = $Category 
				and Equipo_ID > 0 and Activo = 1
			order by Equipo_FULLDESC asc
			limit 1;";
	$result = $Config->query($sql4);
	if ($result->num_rows > 0) {
		// output data of each row
		while($row2 = $result->fetch_assoc()) {
			$team = $row2["Equipo_ID"];
	...
	$sql5 = "SELECT Equipo_ID, Equipo_FULLDESC 
			FROM $schema.Equipos
			where Torneo_ID = $Season 
				and Fuerza = $Category 
				and Equipo_ID > 0 and Activo = 1
			order by Equipo_FULLDESC asc";
	$result = $Config->query($sql5);
	if ($result->num_rows > 0) {
		$totTeam = $result->num_rows;
	}
	
	$sql6 = "SELECT Equipo_ID, Equipo_FULLDESC
			FROM $schema.Equipos
			where Torneo_ID = $Season 
				and Fuerza = $Category 
				and Equipo_ID = $team
	...
	$sql7 = "SELECT Equipo_ID, Equipo_FULLDESC, Activo
			FROM $schema.Equipos
			where Torneo_ID = $Season 
				and Fuerza = $Category 
				and Equipo_ID <> $team
				and Equipo_ID > 0
				and Activo = 1
			order by Activo, Equipo_FULLDESC asc";
	$result = $Config->query($sql7);
	if ($result->num_rows > 0) {
		// output data of each row
		$selected = false;
		$htmlCat .= '								<ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLinkCat">';
	...
	$htmlCat .= '								</div>
												<div class="col-4 col-sm-4 col-md-2 col-lg-2 col-xl-2 col-xxl-2">
													<div class="dropdowni btn-tooltip" data-bs-toggle="tooltip" data-bs-placement="top">
														<img src="./imagenes/refresh.png" width="20" height="20" onclick="playersManagementAdminCategoryTeamShowReloadList($(\'#playersManagementAdminSelectedCategory\').val(), $(\'#playersManagementAdminSelectedTeam\').val());" style="margin-left: 10;  margin-top: 2px;">
													</div>
												</div>
										</div>
									</div>
	...
	$retunData = array('status' => '1', 'message' => 'Success.', 'dataCategory' => $htmlCat, 'category' => $Category, 'team' => $team, 'sql1' => $sql1, 'sql2' => $sql2, 'sql3' => $sql3, 'sql4' => $sql4, 'sql5' => $sql5, 'sql6' => $sql6, 'sql7' => $sql7);
	$Config->Close();
