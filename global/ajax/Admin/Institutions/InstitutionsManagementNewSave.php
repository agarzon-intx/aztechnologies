...
unset($__i, $__prev, $__base, $__inc, $__app_here);
	...
	$sessionstat = $fgmembersite->CheckLogin('InstitutionsManagementNewSave.php');
	...
	$retunData = array('status' => '0', 'message' => 'No insert.', 'dataInstitutionAnswer' => 'Error');
	...
	$sql1 = "INSERT INTO $schema.Instituciones
				(Institucion_DESC, Activo, Logo, Institucion_FULLDESC, Torneo_ID, Institucion_DESC5)
			 VALUES
				('$descripcion', $estatus, '', '$descripcionLarga', $Season, '$desc5')";
	$result = $Connection->query($sql1);
	if ($result) {
		$newId = (int) $Connection->insert_id;
		$logoKey = 'I-' . $Season . '-' . $newId;
		$Connection->query("UPDATE $schema.Instituciones SET Logo = '" . $Connection->real_escape_string($logoKey) . "' WHERE Institucion_ID = $newId AND Torneo_ID = $Season");
		$retunData = array('status' => '1', 'message' => 'Success.', 'dataInstitutionAnswer' => $lang['inst010']);
