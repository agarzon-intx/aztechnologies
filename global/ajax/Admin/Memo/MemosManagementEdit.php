...
unset($__i, $__prev, $__base, $__inc, $__app_here);
	...
	$memo_id = SanitizeInteger($_POST['memo']);
	...
	$sql="	SELECT Minuta_ID, 
				...
				Minuta_Titulo
			FROM $schema.Minutas
			where Minuta_ID = $memo_id;";
	$result = $Config->query($sql);
	if ($result->num_rows > 0) {
		while($row2 = $result->fetch_assoc()) {
			$memoid = $row2["Minuta_ID"]; 
			$Fecha = $row2["Minuta_Fecha"]; 
			$Contenido = $row2["Minuta_Contenido"]; 
			$Titulo = $row2["Minuta_Titulo"]; 
		}
	}
	...
	$htmlMemos .= '<div class="container-fluid py-2">
												...
												<div class="form-check col-12 col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">
													<span style="text-align: center; ">
														<div style="width: 300px; height: 400px; background-color: #BFBFBF; margin: auto;">
															<img id="Minuta" src="imagenes/Minutas/' . $memoid. '.png?tmp=' . $fecha->getTimestamp() . '" alt="Logo" width="300" height="400" />
														</div>
													</span>
												</div>
											</div>
					...
					<div class="row">
						<div class="my-3" >
							<button type="button" class="btn btn-primary" onClick="validateMemoEdit(' . $memoid . ');" >' . $lang['0000'] . '</button>
							<button type="button" class="btn btn-primary" onClick="memoManagementHideEdit();" >' . $lang['0001'] . '</button>
						</div>
					</div>
				</div>
				<script>
	...
	$retunData = array('status' => '1', 'message' => 'Success.', 'dataMemoEdit' => $htmlMemos);
    $Config->Close();
