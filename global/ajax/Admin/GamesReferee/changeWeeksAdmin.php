...
unset($__i, $__prev, $__base, $__inc, $__app_here);
	...
	if($result02){
		...
		if ($result02->num_rows > 0) {
			...
			while($row02 = $result02->fetch_assoc()) {
				$htmlWeeks .= '<li><a class="dropdown-item" onclick="loadWeekAdminR(' . $currentWeek . ',\'' . $row02["Equipo_FULLDESC"] . '\', $(\'#byTeam\').prop(\'checked\') ? 1 : 0);">' . $row02["Equipo_FULLDESC"] . '</a></li>';
			}
			$htmlWeeks .= '</ul>';
		}
	}
	...
	$htmlWeeks .= '<div class="col-1 col-sm-1 col-md-1 col-lg-1 col-xl-1 col-xxl-1">';
	$htmlWeeks .= '<div >';
	$htmlWeeks .= '<img src="./imagenes/refresh.png" width="20" height="20" onclick="loadWeekAdminR(' . $selectedWeek . ', \'' . $selectedTeam . '\', $(\'#byTeam\').prop(\'checked\') ? 1 : 0);" style="margin-left: 10;  margin-top: 2px;">';
	$htmlWeeks .= '</div>';
	$htmlWeeks .= '</div>';
	$htmlWeeks .= '</div>';
	$htmlWeeks .= '</div>';
