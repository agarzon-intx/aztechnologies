...
$schema = $Config->getSchema();
	...
	if ($result->num_rows > 0) {
		...
		while($row2 = $result->fetch_assoc()) {
			$htmlCategories .= '<li><a class="dropdown-item" onclick="playersManagementTeamCategoryShowReloadList(' . $row2["Categoria_ID"] . ')">' . $row2["Categoria_Desc"] . '</a></li>';
		}
		$htmlCategories .= '</ul>';
	} else {
	   $htmlCategories .= "";
	...
	$htmlCategories .= '<input type="hidden" id="playersManagementTeamSelectedCategory" value="' . $Category . '">';
