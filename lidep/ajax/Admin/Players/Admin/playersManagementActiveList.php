						...
						<div class="table-responsive">
							<table class=" table align-items-center mb-0" style="border-color: #136aeb;">
								<thead class="">
									...
									<th scope="col" class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 lh-1" style="width: 105px !important;white-space: normal;padding: 0.75rem 0.2rem;"></span></th>
									<th scope="col" class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 lh-1" style="width: 105px !important;white-space: normal;padding: 0.75rem 0.2rem;">' . $lang['950'] . '</span></th>
								</thead>
								<tbody>';
...
$sql2 = "	SELECT distinct Jugador_ID,
				...
				case when Validado = 0 then '" . $lang["941"] . "'
					...
					'' strikei, 
					'' strikef,
					case when a.Foto is null then '' else 'si' end as Picture
...
if ($result2->num_rows > 0) {
	...
	while($row2 = $result2->fetch_assoc()) {
	    $hasLogo = '';
	    if($row2["Picture"] == ''){
	        $hasLogo = '<img onclick="playersManagementAdminShowEdit(5546)" src="./imagenes/FaltaFoto.png" width="20" height="20" alt="">';
	    }
		if (($count % 2) == 1){
			$htmlPlayer .= "<tr>";
		}else{
		...
		$htmlPlayer .= '<td scope="row" class="align-middle text-left" hidden="true"><span class="text-secondary text-xs font-weight-normal">' . $row2["Jugador_ID"] . '</span></td>
						...
						<td scope="row" class="align-middle text-left"><span class="text-secondary text-xs font-weight-normal">' . $row2["strikei"] . $row2["Nombre"] . ' ' . $row2["Apellido_P"] . ' ' . $row2["Apellido_M"] . $row2["strikef"]  . $hasLogo . '</span></td>
		...
		$htmlPlayer .= '<td scope="row" class="align-middle text-center" style="background: ' . $color . '"><span class="text-secondary text-xs font-weight-normal">' . $row2["strikei"]  . $row2["CurpValida"] . $row2["strikef"]  . '</span></td>
						...
						<td scope="row" class="align-middle text-center" style="background:#fff38f"><span class="text-secondary text-xs font-weight-normal">';
						if($row2["Validado"] == $lang["940"]){
							$htmlPlayer .= '<input type="checkbox" class="playersManagementAdminImprimir" id="playersManagementAdminImprimir' . $row2["Jugador_ID"] . '" name="playersManagementAdminImprimir' . $row2["Jugador_ID"] . '" value="' . $row2["Jugador_ID"] . '"/>';
						}
		$htmlPlayer .= '</span></td>';
		$htmlPlayer .= '</tr>';
		$count++;
...
$htmlPlayer .= '			<tr>
								<td colspan="2" scope="row" class="align-middle text-center">
									<span class="text-secondary text-xs font-weight-normal">
										<button type="button" class="btn btn-primary" onClick="playersManagementAdminShowCreate(' . $Category . ',' . $Team . ');" >' . $lang['0013'] . '</button></span></td>
                                <td colspan="3" scope="row" class="align-middle text-center">
								    <a hidden id="downloadPlayersIDBtn1" href="pdf/registrosEquipo.php?Equipo_ID=' . $Team . '&Edad1=0&Edad2=150&Imprimir=1" target="_blank" download ></a>
								    <span class="text-secondary text-xs font-weight-normal">
								        <button type="button" class="btn btn-primary" onClick="$(\'#downloadPlayersIDBtn1\')[0].click();">' . $lang['915'] . '</button>
								    </span>
								</td>
								<td colspan="2" scope="row" class="align-middle text-center"><span class="text-secondary text-xs font-weight-normal"><button type="button" class="btn btn-primary" onClick="playersManagementValidateAllPlayers(' . $Category . ',' . $Team . ');">Validar Todos</button></span></td>
								<td colspan="4" scope="row" class="align-middle text-center" style="background:#fff38f"><span class="text-secondary text-xs font-weight-normal"><button type="button" class="btn btn-primary" onClick="playersManagementAdminAddPrintList();">' . $lang['952'] . '</button></span></td>
								</thead>
						</table>
					</div>
				</div>
...
if ($result2->num_rows > 0) {
	...
	while($row2 = $result2->fetch_assoc()) {
		...
		$htmlPlayer .= '		<div style="width: 15%;text-align: center;padding-top: 0px;">
									<p style="margin-bottom: 0rem !important;"><span class="text-secondary text-xs font-weight-bold">' . '123' . '</span></p>
								...
								<div style="width: 20%;text-align: center;padding-top: 0px; background:#fff38f">
									<p style="margin-bottom: 0rem !important;"><span class="text-secondary text-xs font-weight-bold">';
		if($row2["Validado"] == $lang["940"]){
			$htmlPlayer .= '<input type="checkbox" class="playersManagementAdminImprimir" id="playersManagementAdminImprimir' . $row2["Jugador_ID"] . '" name="playersManagementAdminImprimir' . $row2["Jugador_ID"] . '" value="' . $row2["Jugador_ID"] . '"/>';
		}
		$htmlPlayer .= '			</span></p>
...
$htmlPlayer .= '			<tr>
								<td scope="row" style="padding-top: 0px; padding-bottom: 0px;">
									<div class="d-flex px-0 py-0">
										<div style="width: 50%;text-align: center;padding-top: 7px;">
											<span class="text-secondary text-xs font-weight-normal">
												<button type="button" class="btn btn-primary" onClick="playersManagementAdminShowCreate(' . $Category . ',' . $Team . ');" >' . $lang['0013'] . '</button>
											</span>
										</div>
										<div style="width: 50%;text-align: center;padding-top: 7px; background:#fff38f">
											<span class="text-secondary text-xs font-weight-normal">
									...
									<div class="d-flex px-0 py-0">
										<div style="width: 50%;text-align: center;padding-top: 0px;">
											...
											<select onChange="printID2();" style="width:150px" name="printRegisters2" id="printRegisters2" size="1">
												...
												<option value="pdf/registrosEquipo.php?Equipo_ID=' . $Team . '&Edad1=0&Edad2=150&Imprimir=1">' . $lang['js915'] . '</option>';
$sql = "SELECT Range_Name, concat('pdf/registrosEquipo.php?Equipo_ID=" . $Team . "&Edad1=', Range_Start,'&Edad2=', Range_End,'&Imprimir=1') url 
...
$htmlPlayer .= '							</select>
										...
										<div style="width: 50%;text-align: center;padding-top: 7px;>
											<span class="text-secondary text-xs font-weight-normal">
												<button type="button" class="btn btn-primary" onClick="playersManagementValidateAllPlayers(' . $Category . ',' . $Team . ');">Validar Todos</button>
											</span>
										</div>
									</div>
								</td>
