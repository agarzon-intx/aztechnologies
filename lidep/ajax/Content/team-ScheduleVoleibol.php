            <?php
				...
				$sql2 = "select distinct * from (
								select 0 as VisitanteS, 
										...
										jo.Jornada_DescCorta as Jornada_D, 
										j.Juego_ID as juego, 
										jugado, 
										concat(l.equipo_desc,'')  as 'Local',
										concat(l.Equipo_DESC3,'')  as 'Local3',
										...
										case 
											when j.Visitante_Id is null then CONCAT(concat(l.Torneo_ID,'-', l.Equipo_ID),l.equipo_desc,' " . $lang['654'] . "') 
											else Comentarios 
										end as Comentarios, 
										Estatus, 
									...
									from  $schema.Juegos as j 
										...
										join $schema.Jornada as jo on j.Jornada_ID = jo.Jornada_ID
			                            join $schema.Categorias ca on ca.Categoria_ID = $Category and ca.Torneo_Id = $Season and ca.Calendario_Id = jo.Calendario_ID
										left outer join $schema.Juegos_Set s on j.Juego_ID = s.Juego_ID
									where jo.Jornada_ID in (select distinct Jornada_ID from $schema.Juegos where Torneo_ID = $Season)
      										and j.Local_ID = $Team and j.Torneo_ID = $Season
									UNION
									select 1 as VisitanteS, j.Torneo_ID as Torneo, jo.Jornada_ID as Jornada, jo.Jornada_DescCorta as Jornada_D, j.Juego_ID as juego, jugado, 
										...
										case 
											when j.Visitante_Id is null then ''
												...
												concat(v.Torneo_ID,'-', l.Equipo_ID)
										end as Logol, 
										case 
											when j.Visitante_Id is null then ''
											else
									...
									from  $schema.Juegos as j 
			                            ...
			                            join $schema.Categorias ca on ca.Categoria_ID = $Category and ca.Torneo_Id = $Season and ca.Calendario_Id = jo.Calendario_ID
										left outer join $schema.Juegos_Set s on j.Juego_ID = s.Juego_ID
									where jo.Jornada_ID in (select distinct Jornada_ID from $schema.Juegos where Torneo_ID = $Season) and
                                                                                j.Visitante_ID = $Team and j.Torneo_ID = $Season) a
						order by Fecha asc, horario asc, VisitanteS, Torneo, Jornada, Juego;";
				//echo $sql2;
				...
				if ($result2->num_rows > 0) {
					...
					while($row2 = $result2->fetch_assoc()) {
						...
						$htmlTeam .=  '<td scope="row" class="align-middle text-center"><span class="text-secondary text-xs font-weight-normal">' . $row2["Jornada_D"]. '</span></td>
						...
						if($row2["jugado"] == 1){
							...
							$htmlTeam .= '<div style="width: 15px;text-align: right;"><p style="margin-top: 0px;margin-bottom: 0px;color: ' . $colorSL . ';font-size: xx-large;font-weight: 500;">' . $row2["GL"] . '</p></div>
            							<div style="width: 20px;text-align: right;margin-top: 12px;margin-bottom: 0px;color: ' . $colorSL . ';">' . $row2["PL"] . '</div>
            							<div style="width: 10px;text-align: center;margin-top: 12px;margin-bottom: 0px;">' . $row2["marcador"] . '</div>
            							<div style="width: 20px;text-align: left;margin-top: 12px;margin-bottom: 0px;color: ' . $colorSV . ';">' . $row2["PV"] . '</div>
            							<div style="width: 15px;text-align: left;margin-right:2px"><p style="margin-top: 0px;margin-bottom: 0px;color: ' . $colorSV . ';font-size: xx-large;font-weight: 500;">' . $row2["GV"] . '</p></div>';
						}
						if($row2["jugado"] == 0){
						    if($row2["GL"] > $row2["GV"]){
						        $colorSL = 'red';
							...
							$htmlTeam .= '<div style="width: 15px;text-align: right;padding-top: 0px;margin-top: -3px;"></div>
            							<div style="width: 20px;text-align: right;padding-top: 6px;color: ' . $colorSL . ';"></div>
            							<div style="width: 10px;text-align: center;padding-top: 6px;"></div>
            							<div style="width: 20px;text-align: left;padding-top: 6px;color: ' . $colorSV . ';"></div>
            							<div style="width: 15px;text-align: left;padding-top: 0px;margin-top: -3px;margin-right:2px"></div>';
						}
						if($row2["Logov"] != ''){
						...
						if($row2["Logov"] != ''){
        					$htmlTeam .= '</div>
    							...
    							<div style="width: 20%;text-align: center;padding-top: 0px;">
    									...
    									<div class="d-flex px-0 py-0 lh-1"><div style="width: 100%;text-align: center;"><span class="text-secondary text-xs font-weight-normal text-wrap">' . $row2["Jornada_D"]. '</span></div></div></div>
						...
						if($row2["jugado"] == 2){
							$htmlTeam .= '<div style="width: 15px;text-align: center;" class="align-self-center">' . $lang['663'] . '</div>';
						     ...
						     if($row2["jugado"] == 1){
								...
								$htmlTeam .= '	<div class="align-self-center" style="width: 25%; text-align: right;padding-right: 3px; font-size:3vw;">' . $row2["Local"] . '</div>
											<div class="justify-content-center d-flex px-0 py-1" style="width: 50%; text-align: right;padding-right: 3px; font-size:3vw;">
												<div><img src="imagenes/' . $row2["Logol"] . '.png?tmp=' . $fecha->getTimestamp() . '" class="avatar avatar-sm me-0" style="border-radius: 0rem !important;"></div>
												<div style="width: 20px;text-align: center;">
													<p style="margin-top: -4px;color: ' . $colorSL . ';font-size: xx-large;font-weight: 500;margin-bottom: 0px !important;">' . $row2["GL"] . '</p>
												</div>
											...
											<div style="width: 25%; text-align: left; padding-left: 3px; font-size:3vw;" class="align-self-center">' . $row2["Visitante"] .'</div>';
						     }
						     if($row2["jugado"] == 0){
    							$colorSV = 'inherit';
    						    $colorSL = 'inherit';
